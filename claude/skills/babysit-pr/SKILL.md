---
name: babysit-pr
description: Monitors the current branch's pull request until it is merge-ready — polls CI, diagnoses and fixes failures locally, and evaluates review comments — then re-arms itself via /loop so it keeps watching hands-off, pausing only when it needs a push or a decision. Use when the user says "babysit this PR", "watch my PR", "monitor the PR", "keep an eye on CI", "get this PR ready to merge", or runs /babysit-pr.
---
...
- **Gated on you:** **pushing** and **judgment calls**. When a pass produces commits ready to push, or a review comment needs your decision, the pass **stops the loop and reports**. You say "push" (or make the call); the next `/babysit-pr` pushes and resumes watching.

So the loop sustains itself while CI runs or the branch is quiet, and deliberately breaks the moment it needs your push approval or a human decision. This is your existing PR discipline, automated.
...
- **Reply in the thread**, not as a top-level PR comment:
  ...
  ```
- Resolve threads you've addressed via the `resolveReviewThread` GraphQL mutation.
- Commit fixes locally. **→ GATE: pause for push.**
...
- **What needs you** (push approval? a decision?)
...
- **Blocked on your push or a decision** → **stop the loop** and wait. You resume by approving/deciding, then the next `/babysit-pr` picks up.
...
- **Reply in review threads**, not top-level comments. Don't dismiss reviews.
